...
} from "./buscarHorariosDisponiveis";
import {
    enviarMensagemWhatsApp
} from "./enviarMensagemWhatsApp";

type ProcessarMensagemParams = {
    empresaId: number;
    telefone: string;
    texto: string;
};
 ...
 */
function formatarHorarioParaBr(
    data: Date
) {
    return new Intl.DateTimeFormat(
        "pt-BR",
 ...
 */
async function buscarListaServicos(
    empresaId: number
) {
    const servicos =
 ...
 */
function converterDataInformada(
    mensagem: string
) {
    const partes =
        mensagem.split("/");
...
export async function processarMensagemWhatsApp({
    ...
    const sessao =
        await prisma.sessaoWhatsApp.findUnique({
            where: {
                empresaId_telefone: {
                    empresaId,
                    telefone,
    ...
    switch (sessao.etapa) {
        ...
        case "ESCOLHENDO_DATA": {
            ...
            if (!sessao.servicoId) {
                ...
                console.error(
                    "A sessão não possui serviço selecionado."
                );

                await prisma.sessaoWhatsApp.delete({
                    where: {
                        empresaId_telefone: {
                            empresaId,
                            telefone,
        ...
        case "ESCOLHENDO_HORARIO": {
            ...
            if (
                ...
                console.error(
                    "A sessão não possui serviço ou data selecionada."
                );
