...
type BuscarHorariosDisponiveisParams = {
    empresaId: number;
    servicoId: number;
    data: string;
};
...
function validarFormatoData(data: string) {
...
function criarDataHora(
    ...
    return new Date(
        `${data}T${horario}:00${FUSO_HORARIO}`
    );
}
...
export async function buscarHorariosDisponiveis({
    ...
    servicoId,
    data,
}: BuscarHorariosDisponiveisParams): Promise<
    HorarioDisponivel[]
> {
    ...
    if (
        ...
        throw new Error(
            "Empresa, serviço ou data inválidos"
        );
    }
     ...
     */
    const inicioDoDiaExcecao =
        criarDataHora(data, "00:00");

    const fimDoDiaExcecao =
        criarDataHora(data, "23:59");
    ...
    if (excecao) {
        ...
        throw new Error(
            "Tipo de disponibilidade do serviço inválido"
        );
    }
    ...
    if (
        periodosFuncionamento.length === 0
    ) {
        return [];
    }
